...
class ResultsTab(QWidget):
    ...
    def init_ui(self):
        ...
        generate_report_btn = QPushButton("🖨️ Générer rapport complet")
        generate_report_btn.setProperty("class", "success")
    ...
    def update_results(self):
        ...
        self.results_text.append(self.build_system_characteristics())
        self.results_text.append(self.build_optimal_solution())
        self.results_text.append(self.build_pump_configuration())
        self.results_text.append(self.build_npsh_verification())
        
        if self.main_window.economic_data:
            self.results_text.append(self.build_economic_analysis())
        
        self.results_text.append(self.build_footer())
    ...
    def build_optimal_solution(self):
        ...
        if hasattr(sol, 'score'):
            section += f"│ Score                               : {sol.score:.4f} / 10\n"
    ...
    def build_economic_analysis(self):
        ...
        section += f"│ Coût configuration pompage           : {data['cost_pump_config']:>15,.2f} {currency}\n"
        section += f"│ ─────────────────────────────────────────────────────────────────────────────────────────\n"
        section += f"│ C_SYSTÈME TOTAL                      : {data['cost_systeme']:>15,.2f} {currency}\n"
        section += "└────────────────────────────────────────────────────────────────────────────────────────────┘\n\n"
        return section
    ...
    def export_json(self, filename):
        try:
            ...
            if self.main_window.economic_data:
                ed = self.main_window.economic_data
                data['analyse_economique'] = {
                    'currency':  ed['currency'],
                    'powers_kW': {
                        'P_hyd':   ed['powers']['P_hyd_kW'],
                        'P_pump':  ed['powers']['P_pump_kW'],
                        'P_total': ed['powers']['P_total_kW']
                    },
                    'costs': {
                        'pipe':        ed['cost_pipe'],
                        'pump_config': ed['cost_pump_config'],
                        'total':       ed['cost_systeme']
                    }
                }
    ...
    def build_final_solution_section(self):
        ...
        if self.main_window.economic_data:
            ...
            section += f"  • Coût configuration pompage     : {data['cost_pump_config']:,.2f}\n"
            section += f"  • C_SYSTÈME TOTAL                : {data['cost_systeme']:,.2f}\n\n"
